...
const isOwner = [`${setting.ownerNumber}`,"6282281657946@s.whatsapp.net","6282279915237@s.whatsapp.net"].includes(sender) ? true : false
...
"fileName": `Ikyybot-MD ${ngazap(prefix)}`,
...
const listMessage = {
  ...
  footer: '© created by Ikyy official',
...
conn.sendMessage(from, { caption: cap, image: { url: feta.data.qris }, buttons: buto, footer: '© created by Ikyy official' })
}
console.log(feta)
}
break
...
if (!q) return reply('link githubnya mana?\n*Contoh:*\n#gitclone https://github.com/diki6969/kyyfes')
var linknya = q
...
if (cekUser("id", sender) == null) return reply(mess.OnlyUser)
if (!q) return reply(`*Contoh:*\n${prefix + command} text`)
var anu = await getBuffer(`https://api.lolhuman.xyz/api/${command}?apikey=SGWN&name=${q}`)
reply(mess.wait)
...
.then(data => {
var opt = { packname: 'IkyyBOT MD', author: 'By Ikyy' }
